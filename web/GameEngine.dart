        ...
        if (to != null) {
            ...
            if (physicsEnabled) {
                ...
                if (sprite.isFull() && level.current != null) {
                    ...
                    window.localStorage['total_stars'] = (int.parse(window.localStorage['total_stars']) + (nr - or)).toString();
...
// No sense to save empty states, indeed
        if (ready && !cards.isEmpty && level.current != null) {
            window.localStorage['level_' + level.chapter.toString() + '_' + level.current.index.toString()] = LevelSerializer.toJSON(cards, bobbin.list, physicsEnabled);
        }
    }
