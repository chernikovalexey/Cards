...
class ComplexPolygonShape extends Shape {
    static final int MAX_POLYGON_VERTICES = 12;
    ...
    ComplexPolygonShape() :super(ShapeType.POLYGON, Settings.POLYGON_RADIUS), vertexCount = 0, vertices = new List<Vector2>(MAX_POLYGON_VERTICES), normals = new List<Vector2>(MAX_POLYGON_VERTICES), centroid = new Vector2.zero() {
     ...
     */
    ComplexPolygonShape.copy(PolygonShape other) :super(ShapeType.POLYGON, other.radius), vertexCount = other.vertexCount, vertices = new List<Vector2>(MAX_POLYGON_VERTICES), normals = new List<Vector2>(MAX_POLYGON_VERTICES), centroid = new Vector2.copy(other.centroid) {
    ...
    Shape clone() => new ComplexPolygonShape.copy(this);
    ...
    void setFrom(List<Vector2> otherVertices, int count) {
        assert (2 <= count && count <= MAX_POLYGON_VERTICES);vertexCount = count;

        // Copy vertices.
        for (int i = 0; i < vertexCount; ++i) {
            assert(vertices[i] != null);vertices[i].setFrom(otherVertices[i]);
        }
        ...
        for (int i = 0; i < vertexCount; ++i) {
            ...
            assert (edge.length2 > Settings.EPSILON * Settings.EPSILON);Vector2_crossVectorAndNumToOut(edge, 1.0, normals[i]);
            normals[i].normalize();
        }
    ...
    void computeCentroidToOut(List<Vector2> vs, int count, Vector2 out) {
        ...
        // Centroid
        assert (area > Settings.EPSILON);out.scale(1.0 / area);
    }
    ...
    void computeMass(MassData massData, num density) {
        ...
        for (int i = 0; i < vertexCount; ++i) {
            ...
            final num intx2 = k_inv3 * (0.25 * (ex1 * ex1 + ex2 * ex1 + ex2 * ex2) + (px * ex1 + px * ex2)) + 0.5 * px * px;
            final num inty2 = k_inv3 * (0.25 * (ey1 * ey1 + ey2 * ey1 + ey2 * ey2) + (py * ey1 + py * ey2)) + 0.5 * py * py;
        ...
        // Center of mass
        assert (area > Settings.EPSILON);center.scale(1.0 / area);
