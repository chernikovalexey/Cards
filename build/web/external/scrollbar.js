...
var dw_Event = {add: function (obj, etype, fp, cap) {
    cap = cap || false;
    if (obj.addEventListener)obj.addEventListener(etype, fp, cap); else if (obj.attachEvent)obj.attachEvent("on" + etype, fp);
}, remove: function (obj, etype, fp, cap) {
    cap = cap || false;
    if (obj.removeEventListener)obj.removeEventListener(etype, fp, cap); else if (obj.detachEvent)obj.detachEvent("on" + etype, fp);
}, DOMit: function (e) {
    e = e ? e : window.event;
...
dw_Scrollbar_Co = {setBarDims: function (scrollObj, barObj) {
    var bar;
    ...
    if (dw_scrollObj.enableKeyboardScroll && !dw_Util.isTouchDevice()) {
        var bar = document.getElementById(barObj.barId), track = document.getElementById(barObj.trackId);
        bar.tabIndex = 0;
        track.tabIndex = 0;
