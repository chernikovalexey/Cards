...
- PWA: `manifest.webmanifest` + `sw.js` live at the repo root (root scope) and are staged by `npm run deploy`; icons are `web/img/pwa-*.png`. The service worker is **network-first with cache fallback** for all same-origin GETs (fresh deploys are never served stale; the game boots and plays offline after one visit; boot shell precached on install). Bump `VERSION` in `sw.js` only to purge old cache entries. Registration happens from `cards.html` after `load`. Pinned by `tests/pwa.spec.js` (offline boot on chromium).
